  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        <p className="text-base-content/70 text-sm leading-relaxed mb-6 line-clamp-4">
          {blog.content}
        </p>
      </div>

      {/* Footer Info */}
      <div className="pt-6 border-t border-base-content/5 flex items-center justify-between text-xs font-medium text-base-content/60">
        <div className="flex items-center gap-2">
          <div className="size-8 rounded-full bg-primary/10 flex items-center justify-center text-primary overflow-hidden">
            {authorUser?.image ? (
              <img src={authorUser.image} alt={authorUser.name} className="size-full object-cover" />
            ) : (
              <FaUserEdit size={14} />
            )}
          </div>
