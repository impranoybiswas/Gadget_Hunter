...
export default function Sidebar() {
  ...
  return (
    <div
      className={`h-[calc(100dvh-64px)] sticky top-16 bg-gray-900 text-white hidden md:flex flex-col shadow-lg border-r border-gray-700 transition-all duration-300 ease-in-out  ${collapsed ? "w-18" : "lg:w-60 w-50"}`}>
        ...
        {/* Navigation Links */}
        {
          sidebarLinks.map((link) => (
            <Link key={link.name} href={link.href} className={`w-full h-14 flex items-center justify-start hover:bg-gray-800 cursor-pointer border-l-4  ${pathname === link.href ? "bg-gray-800 text-primary border-primary" : "border-transparent"}`}>
