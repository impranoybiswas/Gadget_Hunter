  ...
  const applyAndSavePalette = async (key: string) => {
    ...
    if (session?.user?.email) {
      ...
      try {
        ...
        toast.success(`Theme updated to ${PALETTES[key].name}`, { id: 'theme-save' });
      } catch (error) {
    ...
    <section className="w-full max-w-4xl mx-auto py-6">
      ...
      <div className="mb-10">
        ...
        <div className="flex items-center justify-between mt-2">
          <p className="text-base-content/60 ml-1">Personalize your workspace and system preferences.</p>
      ...
      <div className="grid grid-cols-1 gap-10">
        ...
        <motion.div
          ...
          className="bg-base-100/60 backdrop-blur-xl border border-base-content/10 shadow-xl rounded-3xl p-6 sm:p-10"
        >
          <div className="flex items-center gap-3 border-b border-base-content/5 pb-4 mb-8">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/50">Select a combination to automatically update and save your theme.</p>
            </div>
          </div>
            ...
            {Object.keys(PALETTES).map((key) => (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <span className={`font-bold text-sm ${selectedPalette === key ? "text-primary" : "text-base-content/70"}`}>
                    {PALETTES[key].name}
                  </span>
                  {selectedPalette === key && (
                ...
                <div className="flex gap-2">
                  <div className="size-8 rounded-lg shadow-sm" style={{ backgroundColor: PALETTES[key].primary }} title="Primary"></div>
                  <div className="size-8 rounded-lg shadow-sm" style={{ backgroundColor: PALETTES[key].secondary }} title="Secondary"></div>
                  <div className="size-8 rounded-lg shadow-sm" style={{ backgroundColor: PALETTES[key].accent }} title="Accent"></div>
                </div>
          ...
          {/* Action Buttons */}
          <div className="mt-12 pt-8 border-t border-base-content/5 flex flex-wrap gap-4 justify-end">
            <button
              onClick={handleReset}
